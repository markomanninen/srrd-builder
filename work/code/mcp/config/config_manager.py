...
class ConfigManager:
    ...
    def _apply_config_data(self, config_data: Dict[str, Any]):
        ...
        if "database" in config_data:
            ...
            self.database.sqlite_path = db_config.get(
                "sqlite_path", self.database.sqlite_path
            )
            self.database.enable_wal = db_config.get(
        ...
        if "vector" in config_data:
            ...
            self.vector.persist_directory = vector_config.get(
                "persist_directory", self.vector.persist_directory
            )
            self.vector.embedding_model = vector_config.get(
                "embedding_model", self.vector.embedding_model
            )
            self.vector.max_chunk_size = vector_config.get(
                "max_chunk_size", self.vector.max_chunk_size
            )
            self.vector.chunk_overlap = vector_config.get(
    ...
    def _load_environment_variables(self):
        ...
        if os.getenv("SRRD_VECTOR_DIR"):
            ...
            self.vector.persist_directory = os.path.join(
                project_path, ".srrd", "knowledge.db"
            )
    ...
    def validate_config(self) -> List[str]:
        ...
        if not os.path.exists(self.vector.persist_directory):
            try:
                ...
                issues.append(
                    f"Cannot create vector database directory: {self.vector.persist_directory}"
                )
