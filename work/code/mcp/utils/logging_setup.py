...
class MCPLoggerAdapter:
    ...
    def log_tool_call(
        self, tool_name: str, tool_args: dict, result: any = None, error: str = None
    ):
        ...
        if result is not None:
            ...
            self.logger.error(
                f"Tool call failed: {tool_name} - {error}", extra=log_data
            )
        else:
    ...
    def log_storage_operation(
        self, operation: str, path: str, status: str, details: dict = None
    ):
        ...
        if status == "success":
            self.logger.info(
                f"Storage operation successful: {operation} on {path}", extra=log_data
            )
        else:
            self.logger.warning(
                f"Storage operation failed: {operation} on {path}", extra=log_data
            )
    ...
    def log_performance_metric(self, metric_name: str, value: float, unit: str = ""):
        ...
        self.logger.info(
            f"Performance metric: {metric_name} = {value} {unit}", extra=log_data
        )
...
def create_session_logger(session_id: str, project_path: str = "") -> MCPLoggerAdapter:
    ...
    if project_path:
        session_log_file = os.path.join(
            project_path, "logs", f"session_{session_id}.log"
        )
