...
class ResearchFrameworkService:
    ...
    def calculate_act_completion(self, tools_used: List[str], research_act: str) -> Dict[str, Any]:
        ...
        if not act_tools:
            return {'completion_percentage': 0, 'categories_completed': 0, 'tools_used': 0}
    ...
    def calculate_category_completion(self, tools_used: List[str], category: str) -> Dict[str, Any]:
        ...
        completion_percentage = (len(tools_used_in_category) / len(category_tools)) * 100
    ...
    def recommend_next_tools(self, tools_used: List[str], current_act: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        act_order = ['conceptualization', 'design_planning', 'knowledge_acquisition', 
        ...
        if current_act:
            ...
            unused_current_tools = [tool for tool in current_act_tools if tool not in tools_used]
    ...
    def detect_workflow_gaps(self, tools_used: List[str]) -> List[Dict[str, Any]]:
        ...
        for act_name, act_data in self.acts.items():
            ...
            for category in act_data['categories']:
                category_completion = self.calculate_category_completion(tools_used, category)
                if category_completion['completion_percentage'] == 0:
                    gaps.append({
                        ...
                        'severity': 'high' if act_name in ['conceptualization', 'design_planning'] else 'medium',
    ...
    def generate_research_summary(self, tools_used: List[str]) -> Dict[str, Any]:
        ...
        for act_name in self.acts.keys():
            summary['acts_progress'][act_name] = self.calculate_act_completion(tools_used, act_name)
        
        # Calculate progress for each category
        for category_name in self.categories.keys():
            summary['categories_progress'][category_name] = self.calculate_category_completion(tools_used, category_name)
