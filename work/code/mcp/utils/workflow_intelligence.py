...
class WorkflowIntelligence:
    ...
    async def analyze_research_progress(self, project_id: int) -> Dict[str, Any]:
        ...
        framework_summary = self.research_framework.generate_research_summary(tools_used)
        ...
        analysis = {
            ...
            'workflow_health': await self._assess_workflow_health(project_id, tools_used),
    ...
    async def generate_recommendations(self, project_id: int, session_id: int) -> List[Dict[str, Any]]:
        ...
        for rec in framework_recs:
            enhanced_rec = await self._enhance_recommendation(project_id, rec, tools_used)
    ...
    async def detect_milestones(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        for act_name in self.research_framework.acts.keys():
            completion = self.research_framework.calculate_act_completion(tools_used, act_name)
            ...
            if completion['completion_percentage'] >= 100:
                milestone = {
                    ...
                    'tools_involved': await self._get_tools_for_act(project_id, act_name),
                    'impact_score': 5,
        ...
        acts_touched = len(set(self.research_framework.get_tool_research_context(tool)['act'] 
    ...
    async def calculate_research_velocity(self, project_id: int) -> Dict[str, Any]:
        ...
        tool_usage_data = await self.sqlite_manager.get_research_progress_summary(project_id)
        ...
        recent_usage = [u for u in tool_usage 
                       if datetime.fromisoformat(u[10].replace('Z', '+00:00')) >= recent_cutoff]
        previous_usage = [u for u in tool_usage 
                         if previous_cutoff <= datetime.fromisoformat(u[10].replace('Z', '+00:00')) < recent_cutoff]
        ...
        return {
            ...
            'estimated_completion_days': round(estimated_days) if estimated_days else None,
    ...
    async def _assess_workflow_health(self, project_id: int, tools_used: List[str]) -> Dict[str, Any]:
        ...
        if not act_usage:
            ...
            variance = sum((count - expected_per_act) ** 2 for count in act_usage.values()) / 6
        ...
        if medium_severity_gaps > 0:
            health_issues.append(f"{medium_severity_gaps} medium-severity workflow gaps")
    ...
    async def _enhance_recommendation(self, project_id: int, recommendation: Dict[str, Any], 
        ...
        if velocity['velocity_trend'] == 'decelerating':
            enhanced_reasoning += ". Consider focusing on this tool to maintain research momentum."
        ...
        tool_context = self.research_framework.get_tool_research_context(recommendation['tool'])
        if tool_context:
            act = tool_context['act']
            act_completion = self.research_framework.calculate_act_completion(tools_used, act)
            
            if act_completion['completion_percentage'] < 25:
                enhanced_reasoning += f" This is an early step in {tool_context['act_name']}."
            elif act_completion['completion_percentage'] > 75:
                enhanced_reasoning += f" This will help complete the {tool_context['act_name']} phase."
        
        # Add estimated effort
        effort_estimate = "low"  # Could be enhanced with ML model
        if recommendation['tool'] in ['generate_latex_document', 'simulate_peer_review', 'build_knowledge_graph']:
            effort_estimate = "high"
        elif recommendation['tool'] in ['clarify_research_goals', 'suggest_methodology']:
    ...
    async def generate_session_summary(self, session_id: int) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(u['execution_time_ms'] for u in successful_tools if u['execution_time_ms']) / len(successful_tools) if successful_tools else 0
