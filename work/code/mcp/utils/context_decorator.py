...
from current_project import get_current_project, get_current_database_path, has_current_project
...
async def log_tool_usage(tool_name: str, parameters: Dict[str, Any], project_path: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if is_async:
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                if not allow_explicit_project_path:
                    ...
                    if provided_project_path:
                        logger.debug(f"project_path parameter is deprecated, using current project: {current_project}")
                # If allow_explicit_project_path is True, do not pop, just pass through
                # Check if context is required
                if require_context and not (current_project or kwargs.get('project_path')):
                    error_msg = (
                ...
                if enable_logging and (current_project or kwargs.get('project_path')):
                    await log_tool_usage(func.__name__, kwargs, kwargs.get('project_path') or current_project)

                # Call the function
                if not (current_project or kwargs.get('project_path')) and fallback_message:
            ...
            def sync_wrapper(*args, **kwargs):
                ...
                if enable_logging and (current_project or kwargs.get('project_path')):
                    try:
                        asyncio.create_task(log_tool_usage(func.__name__, kwargs, kwargs.get('project_path') or current_project))
                    except RuntimeError:
                        pass
...
def project_required(fallback_message: Optional[str] = None, enable_logging: bool = True) -> Callable:
    """
    Convenience decorator for tools that require project context
    Equivalent to @context_aware(require_context=True)
    """
    return context_aware(require_context=True, fallback_message=fallback_message, enable_logging=enable_logging)

def context_optional(fallback_message: Optional[str] = None, enable_logging: bool = True) -> Callable:
    """
    Convenience decorator for tools that can work with or without context
    Equivalent to @context_aware(require_context=False)
    """
    return context_aware(require_context=False, fallback_message=fallback_message, enable_logging=enable_logging)
