...
class ProjectManager:
    def __init__(self, project_path: str):
        ...
        db_path = SQLiteManager.get_db_path(self.project_path)
        logger.info(f"[ProjectManager] Using sessions.db path: {db_path}")
        db_path = SQLiteManager.get_sessions_db_path(str(self.project_path))
        logger.info(f"[ProjectManager] Using sessions.db path: {db_path}")
        self.sqlite_manager = SQLiteManager(db_path)
        vector_db_path = str(self.project_path / ".srrd" / "data" / "knowledge.db")
        logger.info(f"[ProjectManager] Using knowledge.db path: {vector_db_path}")
    ...
    async def initialize_project(
        ...
        try:
            ...
            current_project_path = (
                Path(__file__).resolve().parent.parent / "utils" / "current_project.py"
            )
            ...
            switch_error = str(e)
            logger.warning(
                f"Project created successfully, but MCP context switch failed: {switch_error}"
            )
    ...
    def setup_configuration(self, config: Dict[str, Any]) -> bool:
        ...
        full_config = {
            ...
            "mcp_server": {"enabled": True, "project_path": str(self.project_path)},
            "storage": {
                "git_enabled": True,
                "sqlite_db": SQLiteManager.get_sessions_db_path(str(self.project_path)),
                "vector_db": str(self.project_path / ".srrd" / "data" / "knowledge.db"),
            },
