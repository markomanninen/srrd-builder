...
class ClaudeMCPServer:
            ...
            if project_path:
                ...
                if WorkflowIntelligence and self.research_framework:
                    self.workflow_intelligence = WorkflowIntelligence(
                        self.sqlite_manager, self.research_framework
                    )
        ...
        if self.sqlite_manager:
            try:
                ...
                if not project_row:
                    ...
                    project_path = (
                        get_current_project()
                    )  # os.environ.get('SRRD_PROJECT_PATH', '/tmp/default_project')
                ...
                print(
                    f"Warning: Could not ensure default project: {e}", file=sys.stderr
                )
    ...
    async def handle_request(self, request_data):
        ...
        try:
            ...
            if method == "initialize":
                ...
                project_path = (
                    get_current_project()
                )  # os.environ.get('SRRD_PROJECT_PATH')
                server_info = {"name": "SRRD Builder MCP Server", "version": "1.0.0"}
    ...
    async def _execute_tool(self, tool_name: str, tool_args: dict):
        ...
        research_context = (
            self.research_framework.get_tool_research_context(tool_name)
        ...
        try:
            ...
            if self.sqlite_manager and research_context:
                ...
                if project_id:
                    ...
                    await self._update_research_progress(
                        project_id, research_context, tool_name
                    )
    ...
    async def _update_research_progress(
        self, project_id: int, research_context: dict, tool_name: str
    ):
