...
async def explain_methodology(**kwargs) -> str:
    ...
    if methodology_type and methodology_type in tool.methodologies:
        ...
        response["methodology_analysis"] = {
            ...
            "suitability_for_domain": methodology_type
            in methodology.get("domains", []),
                ...
                suitable_methodologies.append(
                    {
                        ...
                        "suitability_score": _calculate_suitability(
                            research_question, method_info
                        ),
                    }
                )

        response["methodology_analysis"] = {
            "suitable_methodologies": sorted(
                suitable_methodologies,
    ...
    response["user_interaction_required"] = (
        "Please review this methodology analysis. Which aspect would you like to explore further or refine?"
    )
...
async def compare_approaches(**kwargs) -> str:
    ...
    def get_approach_info(approach_name: str) -> Dict:
        ...
        if approach_name in tool.methodologies:
            return {"type": "methodology", "info": tool.methodologies[approach_name]}
        # Check if it's a research design
        elif approach_name in tool.research_designs:
            return {"type": "design", "info": tool.research_designs[approach_name]}
        else:
            return {
                "type": "unknown",
    ...
    comparison["recommendation"] = (
        f"For your research context '{research_context}', consider the trade-offs between {approach_a} and {approach_b}. The choice depends on your specific research question, available resources, and ethical considerations."
    )
    ...
    comparison["user_interaction_required"] = (
        "Please review this approach comparison. Which approach would you like to pursue, or would you like to explore specific aspects further?"
    )
...
async def validate_design(**kwargs) -> str:
    ...
    missing_components = [
        comp for comp in required_components if comp not in research_design
    ]
    ...
    if "control_group" in research_design or "randomization" in research_design:
        ...
        validity_checks.append(
            "WARNING Internal validity: Consider adding control measures"
        )
    ...
    if "sample" in research_design:
        ...
        if isinstance(sample_info, dict) and sample_info.get("representative"):
            validity_checks.append(
                "PASS External validity: Representative sampling planned"
            )
        else:
            validity_checks.append(
                "WARNING External validity: Ensure representative sampling"
            )
    ...
    if "measurement" in research_design:
        ...
        validity_checks.append(
            "WARNING Construct validity: Define measurement instruments"
        )
    ...
    validation_results["validation_results"][
        "methodological_rigor"
    ] = f"Score: {rigor_score}/7 - {', '.join(rigor_factors)}"
    ...
    if ethical_issues:
        validation_results["validation_results"]["ethical_assessment"] = (
            "WARNING Issues: " + ", ".join(ethical_issues)
        )
    else:
        validation_results["validation_results"][
            "ethical_assessment"
    ...
    if constraints:
        if (
            constraints.get("timeline") == "short"
            and "longitudinal" in str(research_design).lower()
        ):
            feasibility_concerns.append(
                "Timeline may be insufficient for longitudinal design"
            )
        if (
            constraints.get("budget") == "low"
            and "large_sample" in str(research_design).lower()
        ):
            feasibility_concerns.append(
                "Budget constraints may limit large sample recruitment"
            )

    validation_results["validation_results"]["feasibility_analysis"] = (
        "PASS Design appears feasible"
        if not feasibility_concerns
    ...
    if critical_count == 0 and rigor_score >= 5:
        validation_results["approval_status"] = (
            "PASS Ready for implementation with minor refinements"
        )
    elif critical_count <= 2:
        validation_results["approval_status"] = (
            "WARNING Needs revision before implementation"
        )
    else:
        validation_results["approval_status"] = "FAIL Major revisions required"

    # Add user interaction requirement
    validation_results["user_interaction_required"] = (
        "Please review this design validation. What aspect would you like to focus on addressing or improving?"
    )
...
async def ensure_ethics(**kwargs) -> str:
    ...
    if participant_type in ["minors", "vulnerable_populations"]:
        autonomy_issues.append(
            "Special consent procedures needed for vulnerable populations"
        )
    ...
    ethics_review["ethical_framework_analysis"]["analysis"]["beneficence"] = {
        "status": (
            "PASS Adequate" if not benefit_risk_issues else "WARNING Needs attention"
        ),
    ...
    if any(
        word in risk_indicators
        for word in ["invasive", "medication", "psychological_stress"]
    ):
        estimated_risk = "moderate_risk"
    elif any(
        word in risk_indicators for word in ["survey", "interview", "observation"]
    ):
    ...
    if participant_type in ["minors", "vulnerable_populations"]:
        recommendations.append(
            "Obtain specialized ethical approval for vulnerable populations"
        )
    ...
    # Add user interaction requirement
    ethics_review["user_interaction_required"] = (
        "Please review this ethical analysis. Which aspects would you like to address first to ensure ethical compliance?"
    )
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "research_question": {
                    "type": "string",
                    "description": "Research question to address",
                },
                "domain": {"type": "string", "description": "Research domain"},
                "methodology_type": {
                    "type": "string",
                    "description": "Specific methodology to explain (optional)",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "approach_a": {
                    "type": "string",
                    "description": "First research approach",
                },
                "approach_b": {
                    "type": "string",
                    "description": "Second research approach",
                },
                "research_context": {
                    "type": "string",
                    "description": "Research context for comparison",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "research_design": {
                    "type": "object",
                    "description": "Research design to validate",
                },
                "domain": {"type": "string", "description": "Research domain"},
                "constraints": {
                    "type": "object",
                    "description": "Resource or other constraints",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "research_proposal": {
                    "type": "object",
                    "description": "Research proposal to review",
                },
                "domain": {"type": "string", "description": "Research domain"},
                "participant_type": {
                    "type": "string",
                    "description": "Type of participants (adults, minors, vulnerable_populations)",
                    "default": "adults",
                },
            },
