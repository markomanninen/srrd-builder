...
def _determine_challenge_type(challenge_area: str) -> str:
    ...
    if any(word in area_lower for word in ["fundamental", "basic", "foundation"]):
        ...
        word in area_lower for word in ["measurement", "observation", "experimental"]
    ):
        return "methodological_challenge"
    elif any(
        word in area_lower for word in ["interpretation", "meaning", "explanation"]
    ):
...
async def develop_alternative_framework(**kwargs) -> str:
    ...
    if target_phenomena:
        coherence_strengths.append(
            f"Targets {len(target_phenomena)} specific phenomena"
        )
    else:
    ...
    framework_development["coherence_check"] = {
        ...
        "coherence_score": len(coherence_strengths)
        / (len(coherence_strengths) + len(coherence_issues)),
    }
    ...
    framework_development["user_interaction_required"] = (
        "Please review this alternative framework development analysis. What aspect would you like to focus on next?"
    )
...
async def compare_paradigms(**kwargs) -> str:
    ...
    criteria_weights = {
        criterion: 1.0 / len(comparison_criteria) for criterion in comparison_criteria
    }

    comparison["scoring_matrix"] = {
        "criteria_weights": criteria_weights,
        "mainstream_scores": {
            criterion: 0.7 for criterion in comparison_criteria
        },  # Placeholder
        "alternative_scores": {
            criterion: 0.6 for criterion in comparison_criteria
        },  # Placeholder
        "weighted_totals": {"mainstream_total": 0.7, "alternative_total": 0.6},
        "confidence_intervals": {"mainstream": [0.6, 0.8], "alternative": [0.5, 0.7]},
    }
    ...
    # Add user interaction requirement instead of automatic next_steps
    comparison["user_interaction_required"] = (
        "Please review this paradigm comparison analysis. Which direction would you like to pursue based on these findings?"
    )
...
async def validate_novel_theory(**kwargs) -> str:
    ...
    criterion_scores = [
        result["score"] for result in validation_results["validation_results"].values()
    ]
    ...
    if average_score >= 0.8:
        ...
        validation_results["publication_readiness"] = (
            "Needs refinement before submission"
        )
    else:
...
def _validate_logical_consistency(theory: Dict) -> Dict:
    ...
    return {
        ...
        "score": max(score, 0.0),
        "assessment": (
            "Logically consistent" if not issues else f"Issues: {', '.join(issues)}"
        ),
        "improvement_suggestions": (
            ["Formalize mathematical structure", "Check principle compatibility"]
            if issues
            else []
        ),
    }
...
def _validate_explanatory_power(theory: Dict) -> Dict:
    ...
    return {
        ...
        "improvement_suggestions": [
            "Identify more target phenomena",
            "Develop novel explanations",
        ],
    }
...
def _validate_theoretical_elegance(theory: Dict) -> Dict:
    ...
    return {
        ...
        "improvement_suggestions": [
            "Simplify core principles",
            "Enhance mathematical elegance",
        ],
    }
...
def _validate_paradigm_compatibility(theory: Dict) -> Dict:
    ...
    return {
        ...
        "improvement_suggestions": [
            ...
            "Explain paradigm differences clearly",
        ],
    }
...
async def cultivate_innovation(**kwargs) -> str:
    ...
    research_idea = research_context.get("current_state") or kwargs.get(
        "research_idea", "Novel research idea"
    )
    innovation_goals = research_context.get("challenges", []) or kwargs.get(
        "innovation_goals", []
    )
    ...
    innovation_score = sum(
        1 for indicator in innovation_indicators if indicator in research_idea.lower()
    ) / len(innovation_indicators)
    ...
    cultivation_plan["innovation_assessment"] = {
        ...
        "innovation_level": (
            ...
            else "moderate" if innovation_score > 0.1 else "incremental"
        ),
        "paradigm_impact": (
            "potentially_transformative"
            if innovation_score > 0.4
            else "significant" if innovation_score > 0.2 else "modest"
        ),
    }
    ...
    cultivation_plan["development_stages"] = {
        "stage_1_conceptualization": {
            "duration": "2-4 weeks",
            "objectives": [
                "Clarify core insight",
                "Map to existing knowledge",
                "Identify paradigm implications",
            ],
            "deliverables": [
                "Concept document",
                "Literature review",
    ...
    # Add user interaction requirement instead of automatic next_steps
    cultivation_plan["user_interaction_required"] = (
        "Please review this systematic innovation cultivation plan. Which development stage would you like to begin with or focus on?"
    )
...
async def assess_foundational_assumptions(**kwargs) -> str:
    ...
    assessment["vulnerability_assessment"] = {
        "untested_assumptions": [
            ...
            if "untested" in _assess_assumption_testability(ass).lower()
        ],
    ...
    assessment["critical_questions"] = tool.socratic_questions[
        "foundational_assumptions"
    ] + [
    ...
    # Add user interaction requirement instead of automatic next_steps
    assessment["user_interaction_required"] = (
        "Please review this foundational assumptions analysis. Which area of assumptions would you like to explore further or challenge?"
    )
...
async def generate_critical_questions(**kwargs) -> str:
    ...
    # Add user interaction requirement instead of automatic next_steps
    question_framework["user_interaction_required"] = (
        "Please review this comprehensive questioning framework. Which category of questions would you like to work through systematically?"
    )
...
async def evaluate_paradigm_shift_potential(**kwargs) -> str:
    ...
    evaluation["transformation_indicators"] = {
        "paradigm_incompatibility": shift_scores.get("revolutionary_potential", 0.5)
        > 0.7,
    ...
    evaluation["impact_prediction"] = {
        ...
        "technological_implications": _predict_technological_impact(
            theory_framework, domain
        ),
        "societal_consequences": _predict_societal_impact(
            overall_shift_potential, domain
        ),
    ...
    # Add user interaction requirement instead of automatic next_steps
    evaluation["user_interaction_required"] = (
        "Please review this paradigm shift potential evaluation. Based on these results, what direction would you like to pursue?"
    )
...
def _evaluate_shift_metric(theory_framework, metric: str) -> float:
    ...
    if isinstance(theory_framework, str):
        ...
        metric_evaluators = {
            ...
            "revolutionary_potential": lambda t: (
                0.8 if "paradigm" in t.lower() else 0.6
            ),
        }
    else:
        # For dictionary inputs, use more sophisticated evaluation
        metric_evaluators = {
            "explanatory_scope": lambda t: (
                0.8
                if t.get("target_phenomena") and len(t["target_phenomena"]) > 3
                else 0.6
            ),
            "predictive_power": lambda t: 0.7 if t.get("predictions") else 0.5,
            "mathematical_elegance": lambda t: (
                0.8 if t.get("mathematical_basis") else 0.4
            ),
            "empirical_support": lambda t: 0.3,  # Low for novel theories
            "paradigm_coherence": lambda t: 0.7 if t.get("core_principles") else 0.5,
            "revolutionary_potential": lambda t: (
                0.8 if "paradigm" in str(t).lower() else 0.6
            ),
        }
...
def _predict_societal_impact(shift_potential: float, domain: str) -> str:
    ...
    if shift_potential >= 0.8 and domain in [
        "physics",
        "consciousness_studies",
        "biology",
    ]:
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                ...
                "current_paradigm": {
                    "type": "string",
                    "description": "Current paradigm to challenge",
                },
                "challenge_area": {
                    "type": "string",
                    "description": "Specific area of challenge",
                },
                "novel_theory_mode": {
                    "type": "boolean",
                    "description": "Enable novel theory development mode",
                    "default": True,
                },
            },
        ...
        parameters={
            ...
            "properties": {
                ...
                "core_principles": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Core principles of alternative framework",
                },
                "mathematical_basis": {
                    "type": "string",
                    "description": "Mathematical foundation (optional)",
                },
                "target_phenomena": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Target phenomena to explain",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "mainstream_paradigm": {
                    "type": "string",
                    "description": "Mainstream paradigm",
                },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "theory_framework": {
                    "type": "object",
                    "description": "Theory framework to validate",
                },
                "domain": {"type": "string", "description": "Research domain"},
                "validation_criteria": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Validation criteria (optional)",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                "research_idea": {
                    "type": "string",
                    "description": "Research idea to cultivate",
                },
                "domain": {"type": "string", "description": "Research domain"},
                "innovation_goals": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Innovation goals",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                ...
                "current_paradigm": {
                    "type": "string",
                    "description": "Current paradigm to analyze",
                },
            },
    ...
    server.register_tool(
        ...
        parameters={
            ...
            "properties": {
                ...
                "paradigm_context": {
                    "type": "string",
                    "description": "Paradigm context",
                },
                "innovation_level": {
                    "type": "string",
                    "description": "Innovation level (high, moderate, low)",
                    "default": "high",
                },
            },
        ...
        parameters={
            ...
            "properties": {
                ...
                "paradigm_metrics": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Paradigm evaluation metrics (optional)",
                },
            },
            "required": ["theory_framework", "domain"],
        },
