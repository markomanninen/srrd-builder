...
def test_project_context():
    ...
    with tempfile.TemporaryDirectory(prefix="srrd_test_") as temp_dir:
        ...
        # Create config
        config = {"project_name": "Test Project", "domain": "test", "version": "0.1.0"}
...
# Test Classes
@pytest.mark.skipif(
    not METHODOLOGY_TOOLS_AVAILABLE, reason="Methodology tools not available"
)
class TestExplainMethodology:
    ...
    async def test_without_project_path(self):
        ...
        try:
            ...
            assert (
                type(e).__name__ == "ContextAwareError"
            ), f"Expected ContextAwareError, got {type(e).__name__}: {e}"
            # Verify the error message mentions SRRD project context
            assert "SRRD project context" in str(
                e
            ), f"Error message should mention SRRD project context: {e}"

    @pytest.mark.asyncio
    async def test_database_logging_placeholder(
        self, test_database, test_project_context
    ):
    ...
    async def test_methodology_types(self, test_project_context):
        ...
        methodologies = ["quantitative", "qualitative", "mixed_methods", "experimental"]
