...
class TestMethodologyAdvisoryWithLogging:
    ...
    def count_tool_usage_entries(self, db_path: str, tool_name: str = None) -> int:
        ...
        if tool_name:
            cursor = conn.execute(
                "SELECT COUNT(*) FROM tool_usage WHERE tool_name = ?", (tool_name,)
            )
        else:
    ...
    async def test_explain_methodology_with_logging(self, test_project_with_db):
        ...
        ), f"Expected 1 new entry, got {final_count - initial_count}"
    ...
    @pytest.mark.asyncio
    async def test_all_methodology_tools_workflow_with_logging(
        self, test_project_with_db
    ):
        ...
        assert (
            actual_new_entries == expected_new_entries
