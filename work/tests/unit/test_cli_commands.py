...
class TestCLICommands:
    ...
    def setup_git_repo(self, project_dir: Path):
        ...
        subprocess.run(['git', 'config', 'user.email', 'test@srrd-builder.test'], 
                      cwd=project_dir, capture_output=True)
        subprocess.run(['git', 'config', 'user.name', 'SRRD Test'], 
                      cwd=project_dir, capture_output=True)
    ...
    def test_init_command(self):
        ...
        assert (project_dir / 'work' / 'drafts').is_dir(), "work/drafts directory not created"
        assert (project_dir / 'work' / 'research').is_dir(), "work/research directory not created"
        assert (project_dir / 'work' / 'data').is_dir(), "work/data directory not created"
        assert (project_dir / 'publications').is_dir(), "publications directory not created"
        
        # Verify files
        assert (project_dir / '.srrd' / 'config.json').is_file(), ".srrd/config.json not created"
        assert (project_dir / '.gitignore').is_file(), ".gitignore not created"
        
        # Verify config content
        config_file = project_dir / '.srrd' / 'config.json'
        config_content = config_file.read_text()
        assert '"domain": "physics"' in config_content, "Config doesn't contain physics domain"
    ...
    def test_generate_proposal_template(self):
        ...
        self.run_cli_command(['init', '--domain', 'physics', '--template', 'standard'], 
                           cwd=project_dir)
    ...
    def test_generate_paper_template(self):
        ...
        # Initialize project first
        self.run_cli_command(['init', '--domain', 'cs', '--template', 'standard'], 
                           cwd=project_dir)
        
        # Generate paper template
        returncode, stdout, stderr = self.run_cli_command([
            'generate', 'template', 'paper',
            '--title', 'Test Paper'
        ], cwd=project_dir)
    ...
    def test_generate_thesis_template(self):
        ...
        # Initialize project first
        self.run_cli_command(['init', '--domain', 'bio', '--template', 'standard'], 
                           cwd=project_dir)
        
        # Generate thesis template
        returncode, stdout, stderr = self.run_cli_command([
            'generate', 'template', 'thesis',
            '--title', 'Test Thesis'
        ], cwd=project_dir)
    ...
    def test_generate_pdf(self):
        ...
        self.run_cli_command(['init', '--domain', 'physics', '--template', 'standard'], 
                           cwd=project_dir)
        self.run_cli_command(['generate', 'template', 'proposal', '--title', 'Test PDF'], 
                           cwd=project_dir)
    ...
    def test_publish_workflow(self):
        ...
        self.run_cli_command(['init', '--domain', 'physics', '--template', 'standard'], 
                           cwd=project_dir)
        self.run_cli_command(['generate', 'template', 'proposal', '--title', 'Test Publish'], 
                           cwd=project_dir)
    ...
    def test_status_command(self):
        ...
        assert "SRRD Project Status" in stdout or "status" in stdout.lower()
    ...
    def test_init_auto_initializes_git(self):
        ...
        assert returncode == 0, "Init should succeed and auto-initialize git"
        assert "auto-initializing" in stdout.lower() or "git repository initialized" in stdout.lower()
    ...
    def test_publish_fails_missing_draft(self):
        ...
        assert "draft not found" in stdout.lower() or "not found" in stdout.lower()
    ...
    def test_generate_pdf_fails_missing_file(self):
        ...
        returncode, stdout, stderr = self.run_cli_command([
            'generate', 'pdf', 'nonexistent.tex'
        ], cwd=project_dir)
        
        assert returncode != 0, "PDF generation should fail for missing file"
        assert "file not found" in stdout.lower() or "not found" in stdout.lower()
