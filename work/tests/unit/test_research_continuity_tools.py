...
class TestResearchContinuityTools:
    ...
    async def test_get_research_progress_tool(self):
        ...
        try:
            ...
            db_path = SQLiteManager.get_sessions_db_path(str(srrd_dir))
            sqlite_manager = SQLiteManager(db_path)
            await sqlite_manager.initialize()
    ...
    async def test_get_tool_usage_history_tool(self, setup_test_environment):
        ...
        assert (
            "Tool Usage History" in result_session
            or "history" in result_session.lower()
            or "No tool usage history found" in result_session
        )

        # Test with limit
        result_limited = await get_tool_usage_history_tool(limit=1)
        assert (
            len(result_limited) > 0
        ), "Limited tool usage history should return some content"
    ...
    async def test_get_workflow_recommendations_tool(self, setup_test_environment):
        ...
        assert (
            ...
            or "No specific recommendations available" in result
        )
    ...
    async def test_get_research_milestones_tool(self, setup_test_environment):
        ...
        assert (
            "No milestones achieved yet" in result
            or "Achieved Milestones" in result
            or "Recently Detected Milestones" in result
        )
    ...
    async def test_tools_with_invalid_project(self):
        ...
        try:
            ...
            ), "Current project should be None after clearing"
            ...
            try:
                ...
                if type(
                    e
                ).__name__ == "ContextAwareError" and "requires SRRD project context" in str(
                    e
                ):
                    exception_raised = True
                else:
                    pytest.fail(
                        f"Expected ContextAwareError but got {type(e).__name__}: {e}"
                    )
    ...
    async def test_workflow_intelligence_integration(self, setup_test_environment):
        ...
        assert (
            analysis["overall_progress"]["tools_used"] >= 2
    ...
    async def test_research_framework_integration(self):
        ...
        conceptualization_categories = research_framework.acts["conceptualization"][
            "categories"
        ]
        assert (
            "goal_setting" in conceptualization_categories
        )  # Fixed: actual category name
...
class TestResearchContinuityIntegration:
    ...
    async def test_complete_research_workflow(self):
        ...
        try:
            ...
            assert (
                "Workflow Recommendations" in recommendations
                or "No specific recommendations" in recommendations
            )
    ...
    async def test_tool_context_awareness(self):
        ...
        try:
            ...
            try:
                ...
                assert any(
                    error_phrase in result
                    for error_phrase in [
                        "Error:",
                        "error",
                        "context not available",
                        "No project",
                        "requires SRRD project",
                        "not found",
                    ]
                ...
                assert (
                    ...
                    or "project" in str(e).lower()
                    or "found" in str(e).lower()
                ), f"Exception should be context-related, got: {e}"
...
if __name__ == "__main__":
    ...
    async def run_basic_test():
        ...
        try:
            ...
            print(
                "✅ Session start:",
                "SUCCESS" if "Session Started" in result else "FAILED",
            )
            ...
            print(
                "✅ Progress check:",
                "SUCCESS" if "Research Progress" in result else "FAILED",
            )
